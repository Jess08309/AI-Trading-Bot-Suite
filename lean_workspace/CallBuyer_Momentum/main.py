...
class CallBuyerMomentumAlgorithm(QCAlgorithm):
    ...
    MAX_POSITIONS = 3                 # was 4 — fewer, higher-conviction concurrent bets
    ...
    def _daily_scan(self) -> None:
        ...
        for ticker in self.WATCHLIST:
            ...
            if not candidate or not candidate["should_trade"]:
                continue
            # Hard SPY-trend requirement (QC-native proxy for the excluded regime detector) —
            # only buy calls when the broad market itself isn't in a downtrend.
            if spy_closes is not None and len(spy_closes) >= 50:
                if spy_closes[-1] < np.mean(spy_closes[-50:]):
                    continue
    ...
    def _check_exits(self) -> None:
        ...
        for symbol, pos in list(self.positions.items()):
            ...
            if security is None:
                continue
            bid, ask = security.bid_price, security.ask_price
            if bid > 0 and ask > 0:
                current = (bid + ask) / 2
            elif bid > 0:
                current = bid
            else:
                # No live bid (illiquid contract) -- security.price can be a stale
                # last-trade print that masks real deterioration and lets losses
                # run well past STOP_LOSS_PCT before a time-based exit finally
                # forces liquidation at the real (much worse) price. Track how long
