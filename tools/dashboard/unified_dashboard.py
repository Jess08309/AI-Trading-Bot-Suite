...
def _putseller_payload() -> Dict[str, Any]:
    ...
    # Build position list from positions.json
    open_pos = []
    total_credit = 0.0
    total_risk = 0.0
...
function renderSummary(c, a, p, cb, g) {
  ...
  const alpacaDaily = a.balances?.daily_pnl || 0;
  const putsellerDaily = p.balances?.daily_pnl || 0;
  const callbuyerDaily = cb.balances?.daily_pnl || 0;
  const grandDaily = cryptoDaily + alpacaDaily + putsellerDaily + callbuyerDaily;
