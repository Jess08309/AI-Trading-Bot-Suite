...
SIDE_ML_OVERRIDE    = 0.99       # effectively skip SIDE entries
COUNTER_TREND_OVERRIDE = 0.99   # no counter-trend trades
...
MAX_POSITION_PCT    = 0.12       # bigger bets on fewer, better trades
...
def load_6mo_data(tf: int = 1) -> Dict[str, np.ndarray]:
...
def check_exit(pos: Position, price: float, bar: int) -> Optional[str]:
    ...
    if pnl > 0.8 and (pos.peak_pnl_pct - pnl) >= TRAILING_STOP_PCT:  # v6: trigger at 0.8%
        return "TRAILING_STOP"

    # 4. Stale profit decay — peaked above arm threshold, now decaying
    if (pos.peak_pnl_pct >= STALE_ARM_PCT and
        pnl > 0 and
        (pos.peak_pnl_pct - pnl) >= STALE_DECAY_PCT and
        pos.stale_bars >= STALE_MIN_BARS):
        return "STALE_PROFIT_DECAY"

    # 5. Max hold flat
    if hold_bars >= MAX_HOLD_BARS_SPOT and abs(pnl) < MAX_HOLD_FLAT_BAND:
...
def generate_signal(model, sym: str, feat: List[float],
    ...
    # v9: RSI momentum gate — only enter when momentum is accelerating
    ...
    if bar >= rsi_gate_min:
        ...
        if rsi_now < rsi_prev:  # RSI declining = momentum fading
            _diag["rsi_gate"] += 1
            return None
    ...
    if ml_dir > MIN_ENSEMBLE_SCORE:
        # LONG candidate — strict: UP trend + price above short MA
        if trend == "UP" and rsi < MAX_RSI_LONG and current >= ma_short:
            direction = "LONG"
            confidence = ml_conf
        # SIDE with very high confidence only
        ...
        # SHORT candidate — DOWN trend + price below short MA
        if trend == "DOWN" and rsi > MIN_RSI_SHORT and current <= ma_short:
            direction = "SHORT"
            confidence = ml_conf
...
def run_backtest(
    ...
    for bar in range(split_idx, max_bars):
        ...
        for sym in active_symbols:
            ...
            size = kelly_size(balance, confidence, vol, len(positions))
...
def print_report(trades: List[Trade], starting_bal: float, equity_curve: List[float],
    ...
    if shorts:
        ...
        print(f"    SHORT: {len(shorts):4d} trades  {sw/len(shorts)*100:5.1f}% WR  P&L ${sp:+,.2f}")
...
def main():
    ...
    parser.add_argument("--direction", type=str, default="long_only", choices=["both", "short_only", "long_only"])
    ...
    print(f"  Balance: ${args.balance:,.2f}  |  Direction: {args.direction}")
    ...
    all_prices = load_6mo_data(tf=tf)
    if not all_prices:
