...
def _acquire_main_lock() -> None:
    ...
    # Do NOT set SO_REUSEADDR — on Windows it allows multiple processes to bind
    # to the same port simultaneously, defeating the single-instance lock.
    try:
        _main_lock_socket.bind(("127.0.0.1", lock_port))
        _main_lock_socket.listen(1)  # Puts socket in LISTEN state, preventing TIME_WAIT rebind
    except OSError:
