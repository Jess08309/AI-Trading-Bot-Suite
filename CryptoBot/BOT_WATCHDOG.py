...
def check_process_alive() -> bool:
    ...
    try:
        import ctypes
        # Use tasklist to find python.exe OR pythonw.exe running cryptotrades
        for img_name in ("python.exe", "pythonw.exe"):
            result = subprocess.run(
                ["tasklist", "/FI", f"IMAGENAME eq {img_name}", "/FO", "CSV", "/NH"],
...
def check_process_alive_simple() -> tuple[bool, int | None]:
    """Check for python.exe OR pythonw.exe running CryptoBot."""
    try:
        result = subprocess.run(
            ["powershell", "-NoProfile", "-Command",
             "Get-Process python, pythonw -ErrorAction SilentlyContinue | "
             "Where-Object { $_.CommandLine -match 'cryptotrades' } | "
...
def attempt_restart() -> bool:
    ...
    try:
        # Launch bot directly via pythonw.exe (no console window)
        # Set the env vars that START_BOT_AGGRESSIVE.bat would set
        env = os.environ.copy()
        env.update({
            "ENABLE_FUTURES": "true",
            ...
            "DIRECTION_BIAS_STRENGTH": "0.02",
            "RL_SHADOW_MODE": "false",
            "RL_LIVE_SIZE_CONTROL": "false",
            "RL_LIVE_SIZE_MIN_MULT": "0.5",
            "RL_LIVE_SIZE_MAX_MULT": "2.0",
            "USE_LOCKED_PROFILE": "true",
            "LOCKED_PROFILE_PATH": str(SCRIPT_DIR / "data" / "state" / "locked_profile.json"),
            "LOCKED_PROFILE_ALLOW_ENV_OVERRIDES": "true",
            "AGGRESSIVE_FUTURES_BURST": "true",
            "FORCE_MODEL_RETRAIN_ON_START": "true",
            "TRADE_CYCLE_INTERVAL_OVERRIDE": "1",
            "MODEL_RETRAIN_HOURS_OVERRIDE": "2",
            "MIN_ML_CONFIDENCE_OVERRIDE": "0.60",
            "MIN_ENSEMBLE_SCORE_OVERRIDE": "0.58",
            "SIDE_MARKET_FILTER_OVERRIDE": "true",
            "CB_MAX_CONSECUTIVE_LOSSES_OVERRIDE": "5",
            "CB_DAILY_LOSS_LIMIT_PCT_OVERRIDE": "-5",
            "CB_MAX_DRAWDOWN_PCT_OVERRIDE": "-10",
            "MAX_POSITION_PCT_OVERRIDE": "0.15",
            "MAX_POSITIONS_SPOT_OVERRIDE": "12",
            "MAX_POSITIONS_FUTURES_OVERRIDE": "8",
            "MAX_POSITIONS_PER_SYMBOL_SPOT_OVERRIDE": "2",
            "MAX_POSITIONS_PER_SYMBOL_FUTURES_OVERRIDE": "2",
            "MAX_CORRELATION_OVERRIDE": "0.70",
            "FUTURES_LEVERAGE_OVERRIDE": "2",
            "STOP_LOSS_PCT_OVERRIDE": "-3.0",
            "TAKE_PROFIT_PCT_OVERRIDE": "3.0",
            "TRAILING_STOP_PCT_OVERRIDE": "2.0",
            "FUTURES_STOP_LOSS_OVERRIDE": "-4.0",
            "SYMBOL_PAUSE_CONSECUTIVE_LOSSES_OVERRIDE": "4",
        })

        python_exe = SCRIPT_DIR / ".venv" / "Scripts" / "pythonw.exe"
        bot_main = SCRIPT_DIR / "cryptotrades" / "main.py"

        subprocess.Popen(
            [str(python_exe), "-u", str(bot_main)],
            cwd=str(SCRIPT_DIR),
            env=env,
            creationflags=subprocess.CREATE_NO_WINDOW,
        )
        restart_timestamps.append(time.time())
        _log("ACTION", "Bot restarted via pythonw.exe (silent)")
        time.sleep(30)  # give it time to boot

        alive = check_process_alive()
        if alive:
            _log("OK", "Bot confirmed running after restart")
            return True
        else:
