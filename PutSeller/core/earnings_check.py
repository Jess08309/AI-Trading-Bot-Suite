"""
Earnings date checker — avoids selling premium across earnings announcements.
Uses yfinance to look up next earnings date. If yfinance is not installed,
the check is skipped (fails open).
"""
import logging
from datetime import datetime, timedelta
from functools import lru_cache

log = logging.getLogger("PutSeller.earnings")
...
def has_earnings_within(symbol: str, days: int) -> bool:
    ...
    ETFs always return False. If yfinance unavailable, returns False (fails open).
    """
    if _is_etf(symbol):
        return False
    ...
    try:
        ...
        return True  # block trade if we can't verify earnings
    ...
    if 0 <= delta <= days:
        log.info(f"⚠ {symbol}: earnings in {delta}d — SKIPPING")
        return True
