...
class TradingConfig:
    ...
    # Stop Loss / Take Profit  (1:1 ratio — needs only >50% WR)
    STOP_LOSS_PCT: float = -3.0            # -3% stop (room for noise)
    TAKE_PROFIT_PCT: float = 3.0           # 3% target (achievable)
    TRAILING_STOP_PCT: float = 2.0         # 2% trailing (wider, fewer whipsaws)

    # Futures specific
    FUTURES_LEVERAGE: int = 2
    FUTURES_STOP_LOSS: float = -4.0        # -4% for leveraged positions
    ...
    MAX_HOLD_FLAT_BAND_PCT: float = 1.0    # |P/L| < 1% counts as "flat"
    ...
    MIN_ML_CONFIDENCE: float = 0.62        # Model must predict >=62% one direction
    MIN_ENSEMBLE_SCORE: float = 0.58       # Composite ML+trend quality score
    MAX_RSI_LONG: float = 65.0             # Don't buy if RSI > 65 (tighter)
    MIN_RSI_SHORT: float = 35.0            # Don't short if RSI < 35 (tighter)

    # Signal quality filters
    SIDE_MARKET_FILTER: bool = True        # Skip SIDE-trend signals
    SIDE_MARKET_ML_OVERRIDE: float = 0.72  # ML conf above this can override SIDE filter
    COUNTER_TREND_ML_OVERRIDE: float = 0.78  # ML conf above this can trade against trend (heavy penalty)
    MIN_MODEL_TEST_ACCURACY: float = 0.53  # Reject models below this OOS accuracy
    SYMBOL_PAUSE_CONSECUTIVE_LOSSES: int = 4  # Pause symbol after N straight losses
    ...
    # Correlation
    MAX_CORRELATION: float = 0.85          # Tighter correlation limit
...
class MLModel:
    ...
    def _train(self, price_history: Dict[str, List[float]]):
        """Train trend-following model on hourly candles with quality gate."""
        ...
        for symbol, prices in price_history.items():
            ...
            if len(prices) >= 120 and not series_list:
                series_list.append((prices, 0.002, 0.0008))  # 0.2% target, 0.08% flat filter
        ...
        if class_ratio < 0.15 or class_ratio > 0.85:
            self.logger.warning(
                f"Training data heavily skewed ({class_ratio:.1%} positive). "
                f"Model would have no discriminative power. Skipping."
            )
            return
        ...
        candidate = GradientBoostingClassifier(
            ...
            learning_rate=0.05,    # Slower learning, more generalizable
            min_samples_leaf=10,   # Avoid overfitting small patterns
    ...
    def predict(self, prices: List[float]) -> Dict[str, float]:
        ...
        # Confidence = strongest directional probability (0.50 = coin-flip, 1.0 = certain)
...
class TradingBot:
    ...
    def generate_signals(self) -> List[Signal]:
        ...
        if skipped_side or skipped_paused or skipped_confidence or skipped_trend_mismatch:
            self.logger.info(
