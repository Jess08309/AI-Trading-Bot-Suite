...
class TradingConfig:
    ...
    TRADE_CYCLE_INTERVAL: int = 10         # 10 minutes (10 * 60 seconds)
    ...
    MAX_POSITIONS_SPOT: int = 100
    MAX_POSITIONS_FUTURES: int = 100
    MAX_POSITIONS_PER_SYMBOL_SPOT: int = 5
    MAX_POSITIONS_PER_SYMBOL_FUTURES: int = 5
    MAX_POSITION_PCT: float = 0.10         # 10% max per position (was 15%)
    MIN_POSITION_PCT: float = 0.02         # 2% min

    # Stop Loss / Take Profit
    STOP_LOSS_PCT: float = -2.0            # -2% stop (tighter)
    TAKE_PROFIT_PCT: float = 4.0           # 4% target (2:1 ratio)
    TRAILING_STOP_PCT: float = 1.5         # 1.5% trailing
    ...
    STALE_EXIT_PROFIT_ARM_PCT: float = 2.0
    STALE_EXIT_DECAY_PCT: float = 1.0
    STALE_EXIT_STALE_MIN_SPOT: int = 60
    STALE_EXIT_STALE_MIN_FUTURES: int = 30

    # Circuit Breaker
    CB_MAX_CONSECUTIVE_LOSSES: int = 3     # Stop after 3 losses in a row
    CB_DAILY_LOSS_LIMIT_PCT: float = -5.0  # Stop if down 5% daily
    CB_MAX_DRAWDOWN_PCT: float = -10.0     # Stop if down 10% from peak

    # Entry Thresholds - HIGHER for quality
    MIN_ML_CONFIDENCE: float = 0.55        # Temporary aggressive mode (easy to revert)
    MIN_ENSEMBLE_SCORE: float = 0.58       # Temporary aggressive mode (easy to revert)
    MAX_RSI_LONG: float = 70.0             # Don't buy if RSI > 70
    MIN_RSI_SHORT: float = 30.0            # Don't short if RSI < 30
    ...
    # Correlation
    MAX_CORRELATION: float = 0.95          # Temporary: allow highly correlated setups
    ...
    def __post_init__(self):
        ...
        if aggressive_burst:
            ...
            self.MIN_ML_CONFIDENCE = 0.50
            self.MIN_ENSEMBLE_SCORE = 0.52
            self.MAX_CORRELATION = 1.0
            self.CB_MAX_CONSECUTIVE_LOSSES = 20
            self.CB_DAILY_LOSS_LIMIT_PCT = -50.0
            self.CB_MAX_DRAWDOWN_PCT = -80.0
...
class MLModel:
    ...
    def __init__(self, model_path: str = "models/trading_model.joblib"):
        ...
        self.feature_names = [
            ...
            'rsi', 'price_vs_sma20', 'price_vs_sma50',
    ...
    def _train(self, price_history: Dict[str, List[float]]):
        """Train simple trend-following model."""
        ...
        for symbol, prices in price_history.items():
            ...
            for i in range(50, len(prices) - 10):
                ...
                if features is not None:
                    # Target: 1 if price up in next 10 periods, 0 if down
                    future_return = (prices[i + 10] - prices[i]) / prices[i]
                    label = 1 if future_return > 0.001 else 0  # Small buffer
                    X.append(features)
                    y.append(label)
    ...
    def _extract_features(self, prices: List[float]) -> Optional[List[float]]:
        ...
        # Returns
        ret_1 = (p[-1] - p[-2]) / p[-2] if len(p) > 1 else 0
        ret_5 = (p[-1] - p[-6]) / p[-6] if len(p) > 5 else 0
        ret_10 = (p[-1] - p[-11]) / p[-11] if len(p) > 10 else 0

        # Volatility
        vol_10 = np.std(np.diff(p[-10:]) / p[-10:-1]) if len(p) >= 10 else 0.01
        vol_20 = np.std(np.diff(p[-20:]) / p[-20:-1]) if len(p) >= 20 else 0.01
        ...
        # Moving averages
        sma20 = np.mean(p[-20:])
        sma50 = np.mean(p[-50:]) if len(p) >= 50 else sma20
        price_vs_sma20 = (p[-1] - sma20) / sma20
        price_vs_sma50 = (p[-1] - sma50) / sma50

        # Trend
        x = np.arange(min(20, len(p)))
        y = p[-len(x):]
        slope = np.polyfit(x, y, 1)[0] / np.mean(y)

        # Volume proxy (price changes)
        volume_trend = np.mean(np.abs(np.diff(p[-10:])))
    ...
    def predict(self, prices: List[float]) -> Dict[str, float]:
        ...
        if self.model is None or len(prices) < 50:
        ...
        return {
            "direction": up_prob,  # >0.5 is up, <0.5 is down
...
class SentimentAnalyzer:
    """Fixed sentiment analysis - trend following, not contrarian."""
    ...
    def fetch_sentiment(self) -> Tuple[float, Dict[str, float]]:
        ...
        try:
            ...
            self.logger.warning(f"Sentiment fetch failed, using cached/neutral: {e}")
            ...
            self.last_value = cached_global
            return cached_global, cached_coins
...
class TradingBot:
    ...
    def __init__(self):
        ...
        self.enable_futures_data = os.getenv("ENABLE_FUTURES", "false").lower() == "true"
        ...
        self.direction_bias = os.getenv("DIRECTION_BIAS", "short_lean").strip().lower()
        ...
        self.logger.info("=" * 60)
        self.logger.info("PRODUCTION TRADING BOT v4.0")
        ...
        self.logger.info(
            f"Config: aggressive_futures_burst={'on' if os.getenv('AGGRESSIVE_FUTURES_BURST', 'false').strip().lower() == 'true' else 'off'} | "
            f"max_spot={cfg.MAX_POSITIONS_SPOT} | max_futures={cfg.MAX_POSITIONS_FUTURES} | "
            f"per_symbol_spot={cfg.MAX_POSITIONS_PER_SYMBOL_SPOT} | per_symbol_futures={cfg.MAX_POSITIONS_PER_SYMBOL_FUTURES} | "
            f"futures_leverage={cfg.FUTURES_LEVERAGE}x | "
            f"stale_exit={cfg.STALE_EXIT_PROFIT_ARM_PCT:.1f}%->-{cfg.STALE_EXIT_DECAY_PCT:.1f}% "
            f"({cfg.STALE_EXIT_STALE_MIN_SPOT}m/{cfg.STALE_EXIT_STALE_MIN_FUTURES}m)"
        )
    ...
    def _shadow_open_position(self, strategy: str, signal: Signal, size: float, rl_decision: Optional[Dict[str, float]] = None):
        ...
        per_symbol_limit = cfg.MAX_POSITIONS_PER_SYMBOL_FUTURES if signal.symbol.startswith("PI_") else cfg.MAX_POSITIONS_PER_SYMBOL_SPOT
    ...
    def generate_signals(self) -> List[Signal]:
        ...
        signals = []
        aggressive_futures_burst = os.getenv("AGGRESSIVE_FUTURES_BURST", "false").strip().lower() == "true"
        ...
        for symbol in symbols:
            ...
            # Skip low confidence (except aggressive futures burst mode for PI_ symbols)
            if ml_conf < cfg.MIN_ML_CONFIDENCE and not (aggressive_futures_burst and symbol.startswith("PI_")):
                continue
            ...
            # Determine direction - TREND FOLLOWING
            # Long if: ML says up (>0.6), Trend is UP or SIDE, RSI not overbought, Sentiment >= 0
            # Short if: ML says down (<0.4), Trend is DOWN or SIDE, RSI not oversold, Sentiment <= 0
            ...
            if aggressive_futures_burst and symbol.startswith("PI_"):
                direction = "LONG" if ml_direction >= 0.5 else "SHORT"
                confidence = max(cfg.MIN_ML_CONFIDENCE, ml_conf)
            ...
            if direction is None:
                ...
                if ml_direction > long_trigger:
                    # Potential Long
                    if trend == "UP" or (trend == "SIDE" and sent >= -cfg.SENTIMENT_THRESHOLD):
                        if rsi < cfg.MAX_RSI_LONG:
                            direction = "LONG"
                            confidence = (ml_conf + (rsi / 100)) / 2

                elif ml_direction < short_trigger:
                    # Potential Short
                    if trend == "DOWN" or (trend == "SIDE" and sent <= cfg.SENTIMENT_THRESHOLD):
                        if rsi > cfg.MIN_RSI_SHORT:
                            direction = "SHORT"
                            confidence = (ml_conf + ((100 - rsi) / 100)) / 2

            if direction == "SHORT" and self.direction_bias == "short_lean":
                confidence = min(0.999, confidence * (1.0 + self.direction_bias_strength))
            elif direction == "LONG" and self.direction_bias == "short_lean":
                confidence = max(0.0, confidence * (1.0 - self.direction_bias_strength / 2.0))
            elif direction == "LONG" and self.direction_bias == "long_lean":
                confidence = min(0.999, confidence * (1.0 + self.direction_bias_strength))
            elif direction == "SHORT" and self.direction_bias == "long_lean":
                confidence = max(0.0, confidence * (1.0 - self.direction_bias_strength / 2.0))
