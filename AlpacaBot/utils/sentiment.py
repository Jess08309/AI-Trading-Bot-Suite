"""
Market Sentiment Analyzer — Equity/Options Edition
====================================================
Uses ONLY equity-relevant data from Alpaca's own API.
No crypto indices.  No external dependencies beyond what the bot already has.

Sources (in order of weight):
  1. Alpaca News API  (0.40 of per-symbol) — per-symbol Benzinga headlines,
                      last 4 hours, keyword-scored (upgrades/beats/cuts/misses)
  2. VIX via VIXY ETF (0.40 of market-wide) — implied volatility gauge
  3. SPY trend        (0.40 of market-wide) — 5-day + 1-day return vs 20d MA
  4. Market breadth   (0.20 of market-wide) — gainers vs losers from Alpaca screener
  5. Options Flow     (0.35 of per-symbol) — unusual volume/OI ratio detection
                      High V/OI = institutional positioning (call-heavy=bull, put-heavy=bear)

Sentiment score: -1.0 (extreme bearish) to +1.0 (extreme bullish)
Market-wide score cached 5 min; per-symbol news cached 20 min.

Removed: CNN/alternative.me Fear & Greed — crypto-based index, useless for equities.
         Fixed: bar accessor was b.get("close") — SDK returns objects, access is b.close
"""
...
log = logging.getLogger("alpacabot.sentiment")

# ── Keyword scoring lists ─────────────────────────────────────────────────────
# Each match adds ±1 to the raw score; summaries scored at 0.5x weight

BULLISH_WORDS = [
    "upgrade", "upgrades", "upgraded",
    "outperform", "overweight", "buy",
    "beat", "beats", "topped", "exceeded", "surpassed",
    "raises guidance", "raise guidance", "raised guidance", "raised forecast",
...
class MarketSentimentAnalyzer:
    """Multi-source equity market sentiment analyzer."""

    MARKET_CACHE_TTL  = 300   # 5 min for VIX/SPY/breadth/fear-greed
    NEWS_CACHE_TTL    = 1200  # 20 min per-symbol news cache
    ...
    def __init__(self, api_client=None, finnhub_key: str = ""):
        ...
        # Per-symbol news cache: {symbol: (score, fetched_epoch)}
        self._news_cache: Dict[str, Tuple[float, float]] = {}

        # Per-symbol options flow cache: {symbol: (score, fetched_epoch)}
        self._flow_cache: Dict[str, Tuple[float, float]] = {}
        self.FLOW_CACHE_TTL = 600  # 10 min per-symbol flow cache

        # Per-symbol finnhub cache: {symbol: (score, fetched_epoch)}
        self._finnhub_cache: Dict[str, Tuple[float, float]] = {}

        # Lazy-init Alpaca news client
    ...
    def get_sentiment(self) -> float:
        ...
        weights = {"vix": 0.30, "spy_trend": 0.30, "breadth": 0.20, "fear_greed": 0.20}
        ws, wt = 0.0, 0.0
    ...
    def get_per_symbol_adjustment(self, symbol: str,
                                  prices: np.ndarray) -> float:
        """Get per-symbol adjustment combining news sentiment + price momentum
        + options flow.

        Returns: -0.35 to +0.35 adjustment added to global sentiment.
        """
        news_score = self._get_symbol_news_score(symbol)
        mom_score  = self._price_momentum(prices)
        flow_score = self.get_options_flow_signal(symbol)
        finnhub_score = self._get_finnhub_score(symbol)

        # Weight: 30% news, 20% momentum, 30% options flow, 20% finnhub
        combined = (news_score * 0.30 + mom_score * 0.20
                    + flow_score * 0.30 + finnhub_score * 0.20)
        return float(np.clip(combined, -0.40, 0.40))

    def status(self) -> Dict:
        """Return sentiment status for dashboard."""
        return {
            "composite": round(self._composite, 3),
            "sources": {k: round(v, 3) for k, v in self._signals.items()},
            "source_note": "Alpaca News + VIX + SPY + breadth + Fear&Greed + options flow + Finnhub",
            "last_update": (
            ...
            "flow_cache_size": len(self._flow_cache),
            "finnhub_cache_size": len(self._finnhub_cache),
        }
    ...
    def _fetch_vix_signal(self) -> float:
        ...
        if avg <= 0:
            return 0.0
        # Above 5-day avg = elevated fear = bearish signal
        ratio = current / avg
    ...
    def _fetch_market_breadth(self) -> float:
        ...
        try:
            ...
            # Default market_type=MarketType.STOCKS — stocks screener
    ...
    def _fetch_news_score(self, symbol: str) -> float:
        """Fetch Alpaca news for symbol and keyword-score headlines + summaries."""
        ...
        try:
            ...
            if not articles:
                log.debug(f"[News] {symbol}: no articles in last 4h")
                return 0.0
    ...
    # ── Price momentum helper ─────────────────────────────────────────────────

    def _price_momentum(self, prices: np.ndarray) -> float:
        """Short-term vs longer-term momentum from price array."""
        if len(prices) < 20:
            return 0.0
        short_ret = (prices[-1] / prices[-5] - 1) if prices[-5] > 0 else 0.0
        long_ret  = (prices[-1] / prices[-20] - 1) if prices[-20] > 0 else 0.0
        if short_ret > 0 and long_ret > 0:
            return float(min(0.5, (short_ret + long_ret) * 3.0))
        elif short_ret < 0 and long_ret < 0:
            return float(max(-0.5, (short_ret + long_ret) * 3.0))
        return 0.0

    # ── Unusual Options Activity (flow detection) ─────────────────────────────

    def get_options_flow_signal(self, symbol: str) -> float:
        """
        Detect unusual options activity for a symbol by comparing
        today's volume to open interest across the chain.

        High volume/OI ratio = institutional positioning.
        Call-heavy flow = bullish, Put-heavy flow = bearish.

        Returns: -1.0 (extremely bearish flow) to +1.0 (extremely bullish flow)
        Cached 10 minutes per symbol.
        """
        now = time.time()
    ...
    def _fetch_options_flow(self, symbol: str) -> float:
        ...
        Two-step approach:
          1. GET /v1beta1/options/snapshots/{underlying} — volume data
          2. GET /v2/options/contracts?underlying_symbols={symbol} — OI data
        """
        if self.api is None:
            return 0.0
        ...
        try:
            ...
            # Step 1: Snapshots — underlying in URL path (not query param)
            ...
            # Step 2: Contracts for OI — use broker API
            contract_url = f"{self.api.config.BASE_URL}/v2/options/contracts"
            ...
            # Build OI lookup from contracts
            oi_lookup: dict = {}
            ...
            for sym, snap in snapshots.items():
                # Volume from dailyBar (today's traded volume)
                daily_bar = snap.get("dailyBar", {}) or {}
                vol = int(daily_bar.get("v", 0) or 0)

                # OI from contracts lookup
                oi = oi_lookup.get(sym, 0)

                # Determine call vs put from occ symbol
                # Format: UNDERLYING YYMMDD C/P STRIKE (e.g. AAPL260304C00210000)
                ...
                # Flag unusual: volume > 2x OI on this contract
            ...
            # Overall volume/OI ratio (>1.0 = unusually active)
            vol_oi_ratio = total_vol / max(total_oi, 1)

            # Call/Put volume ratio — direction of institutional flow
            if total_vol > 0:
                call_pct = call_volume / total_vol
                # Map: 0.7+ call-heavy = bullish, 0.3- put-heavy = bearish
                direction_signal = (call_pct - 0.5) * 4.0  # ±1.0 range
            else:
                direction_signal = 0.0

            # Intensity multiplier: unusual activity amplifies signal
            intensity = min(2.0, vol_oi_ratio) / 2.0  # 0-1 scale

            # Final score: direction * intensity
    ...
    # ── Per-symbol: Finnhub analyst recs + news ───────────────────────────────
    ...
    def _fetch_finnhub(self, symbol: str) -> float:
        """Fetch Finnhub analyst recommendation trends + company news.

        Analyst recs: strongBuy/buy/hold/sell/strongSell counts -> bullish/bearish ratio
        News: keyword-score recent headlines
        Combined: 60% analyst recs (more reliable) + 40% news sentiment
        """
        try:
            import requests as req
            ...
            # Combine: 60% analyst recs + 40% news
